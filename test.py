...
def standardValueEquivalent(val,tol):
    if val>=1:
      lv = [(val/(10**i)<10 and val/(10**i)>=1) for i in range(0,9)]
      mult = 10**lv.index(True)
      val = val/mult
    if val<1:
      lv = [(val/(10**(-i))<10 and val/(10**(-i))>=1) for i in range(0,13)]
      mult = 10**(-lv.index(True))
      val = val/mult
    ...
    dic3 = {20:1,10:1,5:1,2:2,1:2,0.5:2}
    dif = [ abs(dic[tol][i]-val) for i in range(0,len(dic[tol]))]
    return dic[tol][dif.index(min(dif))]*mult
 #EL PROGRAMA CALCULA VALORES DE RESISTENCIA DESDE 0.001 hasta 955000000 (955M)

def pBand(f,Q,A,C,tolC,tolR1a,tolR1b,tolR2):
    wo = 2*np.pi*f
    CAux = C
    R1aAux = Q/(A*wo*CAux)
    ...
    R1b = standardValueEquivalent(R1bAux,tolR1b)
    R2 = standardValueEquivalent(R2Aux,tolR2)
    R1a = standardValueEquivalent(R1aAux,tolR1a)
    return C, R1a, R1b, R2


C, R1a, R1b, R2 = pBand(1000,10,10,10*10**(-9),1,1,1,1)
print("C: {}\nR1a: {}\nR1b: {}\nR2: {}".format(C,R1a,R1b,R2))
